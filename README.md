...
- **Affiliates** lets Alex add closer codes and unique sell links (`/r/{code}` or `?ref=`). Demo requests and paid checkouts keep that code so Alex can apply Missed-call text-back on the referrer’s Phoenixwebhost account after Stripe succeeds (6 months free, then 1 year at 50% off; then $49 setup + $29/month). There is no automatic Stripe payout and no cash launch-fee payment. The public **Sell with us** page (`/affiliates`, also `/es/affiliates`) explains the pitch.
