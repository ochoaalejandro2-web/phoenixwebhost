...
export async function generateMetadata({
  ...
  if (!isPaFinancialSlug(slug)) return { title: { absolute: "Services" } };
  const query = await searchParams;
  const { client, locale } = await loadClientSite(slug, query);
  return clientPageMetadata(client, locale, "services");
}
...
export default async function ServicesPage({
  ...
  const query = await searchParams;
  const { client, locale } = await loadClientSite(slug, query);
  return renderClientSite(client, contactNotice(query), locale, "services");
}
