...
export default async function PortalLoginPage({
  ...
  return (
    <PortalChrome client={client} locale={locale}>
      <h1 className="font-display text-3xl tracking-tight">{c.loginTitle}</h1>
      <p className="mt-3 max-w-xl text-black/80">
        {c.loginLead(client.businessName, client.phone)}
      </p>
      {!ready ? (
        ...
        <AuthForm slug={slug} mode="login" locale={locale} />
      )}
      <p className="mt-6 text-sm">
        {c.newClient}{" "}
        <Link
          className="font-semibold text-[#00E840]"
          href={withSiteLangPath(portalPath(slug, "/signup"), locale)}
        >
          {c.createAccount}
        </Link>
      </p>
